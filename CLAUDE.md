...
- React 19.2.0
- React Router 7 **framework mode**. Routes are declared in `app/routes.ts`; route
  modules live in `app/routes/`.
- Entry point: `app/root.tsx`. Its `Layout` renders the full HTML document and
  inlines the logo sprite; its default export wraps the app in `ThemeProvider`
  and `IconProvider`.
- ⚠️ `IconProvider` must stay mounted in `app/root.tsx`. Without it, `Icon` falls
  back to the library default `spritePath: "/sprite.svg"`, which nothing serves —
  every icon silently renders an empty `<use>`.
- There is no `index.html` and no `src/main.tsx`; React Router generates the
  document. `src/App.tsx` is a leftover from the pre-migration SPA and is unused.
...
```
app/
  ├── root.tsx          # Document shell + ThemeProvider + IconProvider
  ├── routes.ts         # Route config
  └── routes/           # Route modules

src/
  ├── App.tsx           # UNUSED — leftover from the pre-migration SPA
  ├── components/       # Shared components
  ├── pages/            # Page bodies rendered by the route modules
  ├── recipes/          # Custom Panda recipes
